...
public:
	...
	state() {}
	state(int n, int m, int k, int count) {
		this->n = n;
		this->m = m;
		this->k = k;
		this->count = count;
		...
		for (int i = 0; i < 4; ++i) {
			...
			if (map[nextN][nextM] == '0') {
				if (!visit[nextN][nextM][now.k]) {
					...
					q.push(state(nextN, nextM, now.k, now.count + 1));
				}
			}
			// 다음이 벽이고, 지금까지 부순 벽의 개수가 K보다 작은 경우 이동 가능
			else {
				if (now.k < K && !visit[nextN][nextM][now.k + 1]) {
					visit[nextN][nextM][now.k + 1] = true;
					q.push(state(nextN, nextM, now.k + 1, now.count + 1));
				}
			}
		}
	}
